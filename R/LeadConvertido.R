...
#' Modelo de Machine Learning capaz informar se um Lead (potencial cliente) será convertido e qual a probabilidade de ocorrer?

setwd("C:/Users/saulo/Documents/Cursos/DSA/FCD/BigDataRAzure/Cap11") # Defini o diretório de trabalho.
getwd() # Informa qual o diretório de trabalho.
...
# Variável alvo = 'converteu'

# Gráfico de barras
ggplot(dados_dsa, aes(x=converteu)) + # cria um objeto ggplot, especificando os dados "dados_dsa" e mapeando a variável converteu para o eixo x
  geom_bar(aes(fill=converteu), alpha=0.7) + # adicionamos as barras ao gráfico. geom_bar() cria um gráfico de barras. Dentro de aes(), especificamos fill=converteu, o que significa que cada barra será preenchida com uma cor correspondente ao nível da variável converteu. O alpha=0.7 define a transparência das barras para 70%.
  ggtitle("Distribuição da Variável 'Converteu'") + # título do gráfico
  xlab("Converteu") + # Define o rótulo do eixo x como "Converteu"
  ylab("Contagem") # Define o rótulo do eixo y como "Contagem"
